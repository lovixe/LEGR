...
#define GPSR_MAXJITTER          (HelloInterval.GetSeconds () / 3)
...
RoutingProtocol::SendPacketFromQueue (Ipv4Address dst)
{
  ...
  if(m_neighbors.isNeighbour (dst)) //这里不改也可以,照样可以工作的
    {
      nextHop = dst;
    }
  else{
    nextHop = m_neighbors.GetNextNode (); //只向上发送
...
RoutingProtocol::RecvGPSR (Ptr<Socket> socket)
{
  ...
  m_neighbors.AddIpv4IDTable(sender, hdr.selfID);
  m_neighbors.AddRecv(hdr.selfID, hdr.lp, hdr.lpv, hdr.jump, hdr.recvList);
}
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  ...
  m_neighbors.node = ipv4->GetNetDevice(0)->GetNode();

  m_neighbors.selfID = ipv4->GetNetDevice(0)->GetNode()->GetId();
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      Ptr<Socket> socket = j->first;
      Ipv4InterfaceAddress iface = j->second;
      HelloHeader helloHeader (m_neighbors.selfID);

      helloHeader.lp = m_neighbors.lp;
      helloHeader.lpv = m_neighbors.lpv;
      helloHeader.jump = m_neighbors.jump;
      helloHeader.recvList.clear();

      for(uint64_t i = 0; i < m_neighbors.lastRecv.size(); i++){
        helloHeader.recvList.push_back(m_neighbors.lastRecv[i]);
      }

      m_neighbors.updateOneCycle();
...
RoutingProtocol::AddHeaders (Ptr<Packet> p, Ipv4Address source, Ipv4Address destination, uint8_t protocol, Ptr<Ipv4Route> route)
{
  ...
  if(m_neighbors.isNeighbour (destination))
  ...
  else
    {
      ...
      nextHop = m_neighbors.GetNextNode();
    }
                             ...
                             UnicastForwardCallback ucb, ErrorCallback ecb)
{
  ...
  if (!tHeader.IsValid ())
  {
    std::cout << "valid tHeader! "<< m_neighbors.selfID << "\n";  
  }
  else if(tHeader.Get() == GPSRTYPE_POS)
  {
    p->RemoveHeader (hdr);
    ...
    if(nextHop != Ipv4Address::GetZero ())
    {
      PositionHeader posHeader (0, 0,  0, (uint64_t) 0, (uint64_t) 0, (uint8_t) 0, 0, 0);
      p->AddHeader (posHeader);
      p->AddHeader (tHeader);
