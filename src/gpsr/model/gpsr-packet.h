...
class FDHeader : public Header
{
  ...
  public:
    uint32_t pCount;
    uint32_t idList[15];
    uint32_t jumpList[15];
    Vector dstPosition;
    bool operator== (FDHeader const & o) const;
};

std::ostream & operator<< (std::ostream & os, FDHeader const &);

class HelloHeader : public Header
{
public:
  /// c-tor
  HelloHeader (uint32_t selfID = 0);
  ...
  std::vector<uint32_t> recvList;  //本周期内接受列表
  uint32_t selfID;          //节点ID
  double lp;                //链路压力值
  double lpv;               //链路势能值
